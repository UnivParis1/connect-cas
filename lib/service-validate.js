...
function validateCasResponse(req, res, ticket, casBody, options, next) {
    xml2js(casBody, {explicitRoot: false, tagNameProcessors: [stripPrefix]}, function(err, serviceResponse) {
        ...
        if (serviceResponse.authenticationFailure) {
            const failure = (serviceResponse.authenticationFailure[0] || {})['$'] || {};
            if (failure.code === 'INVALID_TICKET') {
                console.log("ignoring INVALID_TICKET, we will retry with a new ticket");
            } else {
                console.error('CAS server answered ' + failure.code);
                console.error(casBody);
...
function retrievePGTFromPGTIOU(req, pgtIou, cb) {
    req.sessionStore.get(pgtIou, function(err, session){
        if (session) req.session.pgt = session.pgtId;
